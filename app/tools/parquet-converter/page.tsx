...
type ConversionMode = "parquet_to_other" | "csv_to_parquet" | "view_query";
...
export default function ParquetConverterPage() {
  ...
  const handleModeChange = (newMode: ConversionMode) => {
    ...
    if (newMode === "parquet_to_other") {
        setOutputFormat("csv");
    } else if (newMode === "csv_to_parquet") {
  ...
  const handleQuery = async () => {
      ...
      try {
          ...
          // Register file if not already? Actually we should register on upload but simple is to register on execution
          // We need to read file buffer
          const buffer = await file.arrayBuffer();
          // We always call it 'data.parquet' for simplicity in this demo, or file.name
          const fileName = 'data.parquet'; 
          
          await DuckDBClient.registerFile(fileName, new Uint8Array(buffer));
          
          // Run query
          // If the user didn't use the filename provided, we might fail. 
          // Let's force the filename in the query instruction or let them type it.
        ...
        >
            <div className="p-6 sm:p-8 space-y-8">
                ...
                <div className="flex bg-slate-100 p-1 rounded-xl">
                    ...
                    <button
                        onClick={() => handleModeChange("view_query")}
                        className={`flex-1 py-2.5 text-sm font-bold rounded-lg transition-all ${mode === "view_query" ? "bg-white text-slate-900 shadow-sm ring-1 ring-black/5" : "text-slate-500 hover:text-slate-700"}`}
                    >
                        View & Query
                    </button>
                </div>
                ...
                <div className="space-y-4">
                    <label className="block text-sm font-bold text-slate-900 uppercase tracking-wide">
                        1. Upload {mode === "parquet_to_other" || mode === "view_query" ? "Parquet" : "CSV"} File
                    </label>
                    <div className="relative group">
                         <input 
                            type="file" 
                            accept={mode === "parquet_to_other" || mode === "view_query" ? ".parquet" : ".csv"}
                            ...
                            {file ? (
                                ...
                                <div className="flex flex-col items-center gap-3 text-slate-500">
                                    ...
                                    <span className="text-xs opacity-70">
                                        {mode === "parquet_to_other" || mode === "view_query" ? ".parquet files supported" : ".csv files supported"}
                                    </span>
                                </div>
                            )}
                         </div>
